...
const DEFAULT_SERVER_URL = 'https://clipboard.marin.cr';

function getConfigDir(): string {
  return process.env.CLIPCLI_CONFIG_DIR || join(homedir(), '.config', 'clipcli');
}
...
export function getServerUrl(): string {
  if (process.env.CLIPCLI_SERVER_URL) return process.env.CLIPCLI_SERVER_URL;
...
export function getServerUrlSource(): 'environment' | 'config' | 'default' {
  if (process.env.CLIPCLI_SERVER_URL) return 'environment';
  const cfg = loadConfig();
