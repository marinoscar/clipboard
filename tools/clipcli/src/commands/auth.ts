...
export function registerAuthCommands(program: Command): void {
  const auth = program
    .command('auth')
    .description('Manage authentication with the Clipboard API');
  ...
  auth
    ...
    .action(async (_opts, cmd) => {
      ...
      try {
        ...
        output.humanOnly(() => {
          info('Clipboard CLI — Authentication');
          blank();
    ...
    .action(async (_opts, cmd) => {
      ...
      try {
        ...
        if (!authData) {
          output.result(
            { authenticated: false },
            () => info('Not authenticated. Run: clipcli auth login'),
